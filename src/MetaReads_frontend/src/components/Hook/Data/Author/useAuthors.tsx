...
const useAuthors = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const authorResponse: AuthorModel[] = await MetaReads_backend.get_all_author();

      // If authorResponse is not already of type AuthorModel[], you might need to adjust here
      const authorRows: BaseTableColumnProps[] = authorResponse.map((authorData: AuthorModel) =>
        createData({
          id: authorData.id,
